...
- An iOS export preset exists (`export_presets.cfg`, target `ios/pixblast.ipa`); no Android preset is configured. Godot 4.7.2's iOS **simulator** slice is x86_64-only while Xcode 26 simulators are arm64-only, so the simulator cannot run this on Apple Silicon — use a physical device or the macOS build.
