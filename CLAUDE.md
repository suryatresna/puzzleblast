...
- **Rewind** is the only power that looks backwards, and the only one that is not an action: it restores the board **and the tray** to a previous moment. `board.gd` keeps no history — it just hands out plain data via `snapshot()`/`restore()` — because the tray lives in `game.gd`, which owns the ring buffer and pushes an entry before every board-mutating action. Restoring the board without the tray would take the placed piece off the board *and* leave the slot spent.
  ...
  Its effect is a **held state**, not a bang: the backdrop washes to the Time Stone's emerald and `Effects.time_field()` gathers light around the board for `REWIND_GLOW_SECONDS` (3s), then `_restore_flow()` hands the backdrop back to wherever the combo flow had it. `time_field` is the only emitter here that keeps feeding rather than firing once, and it emits **outward** — `%ComboParticles` sits behind the board, so anything aimed inward is hidden under the board's own panel. `_rewind_glow` is held so a second rewind restarts the clock instead of being cut short by the first one's pending restore. Note the halo reads top-and-bottom: the board is 1048px of a 1080px screen, so the side emitters have almost no margin to drift into.
